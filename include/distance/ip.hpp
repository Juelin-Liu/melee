...
namespace melee {
    namespace impl {
        ...
        __force_inline__ dist_t InnerProductResidual(const data_t *pVect1,
                                                     const data_t *pVect2, int dim) {
            dist_t dist{0};
#pragma unroll
            for (auto i = get_main<scale, dimension>(dim);
                 i < get_all<scale, dimension>(dim); i += 1) {
                dist += static_cast<dist_t>(pVect1[i]) * static_cast<dist_t>(pVect2[i]);
            }
            return dist;
        }
...
// K is the unroll factor
        ...
        __force_inline__ float InnerProduct(const float *pVect1, const float *pVect2,
                                            int dim) {
...
#pragma unroll
                ...
                return Sum16(TmpRes) +
                       InnerProductResidual<dist_t, data_t, scale, dimension>(pVect1,
                                                                              pVect2, dim);
            }
...
#pragma unroll
...
#else
  {
    ...
    return InnerProductResidual<dist_t, data_t, scale, dimension>(pVect1,
                                                                  pVect2, dim);
  }
#endif
        }
...
// K is the unroll factor
        template <typename dist_t, typename data_t, int dimension = INT32_MAX>
        __force_inline__ float InnerProduct(const float16 *pVect1,
...
#pragma unroll
                for (int i = 0; i < get_main<scale, dimension>(dim); i += scale) {
                    const auto diff =
                            _mm512_mul_ps(_mm512_cvtph_ps(_mm256_loadu_si256((const __m256i_u *) (reinterpret_cast<const uint16_t *>(pVect1) + i))),
...
// K is the unroll factor
        template <typename dist_t, typename data_t, int dimension = INT32_MAX>
        __force_inline__ int InnerProduct(const uint8_t *pVect1, const uint8_t *pVect2,
                                          int dim) {
 ...
 */
    ...
    __force_inline__ dist_t InnerProduct(const data_t *pVect1, const data_t *pVect2,
        ...
        switch (dim) {
            case 200:
                return impl::InnerProduct<dist_t, data_t, 200>(pVect1, pVect2, dim);
            case 128:
                return impl::InnerProduct<dist_t, data_t, 128>(pVect1, pVect2, dim);
            case 100:
                return impl::InnerProduct<dist_t, data_t, 100>(pVect1, pVect2, dim);
            case 96:
                return impl::InnerProduct<dist_t, data_t, 96>(pVect1, pVect2, dim);
            case 64:
                return impl::InnerProduct<dist_t, data_t, 96>(pVect1, pVect2, dim);
            case 32:
                return impl::InnerProduct<dist_t, data_t, 96>(pVect1, pVect2, dim);
            case 16:
                return impl::InnerProduct<dist_t, data_t, 16>(pVect1, pVect2, dim);
            default:
                return impl::InnerProduct<dist_t, data_t>(pVect1, pVect2, dim);
                ;
        }
    };

    __force_inline__ float InnerProduct(const float *pVect1, const float *pVect2, int dim) {
        return InnerProduct<float, float, float>(pVect1, pVect2, dim);
    };

    __force_inline__ float InnerProduct(const float16 *pVect1, const float16 *pVect2, int dim) {
        return InnerProduct<float, float16, float>(pVect1, pVect2, dim);
    };

    __force_inline__ int InnerProduct(const uint8_t *pVect1, const uint8_t *pVect2, int dim) {
