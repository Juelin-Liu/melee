...
namespace cnpyMmap
{
    ...
    void read_npy_header(FILE *fp, size_t &word_size, std::vector<size_t> &shape, bool &fortran_order)
    {
        char buffer[256];
        size_t res = fread(buffer, sizeof(char), 11, fp);
        if (res != 11)
            throw std::runtime_error("read_npy_header: failed fread");
    ...
    NpyArray npy_load(std::string fname)
    {
        ...
        size_t word_size;
        bool fortran_order;
        read_npy_header(fp, word_size, shape, fortran_order);
        ...
        size_t header_bytes = sb.st_size - arr.num_bytes();
        // std::cout << "header bytes: " << header_bytes << std::endl;
        // std::cout << "array bytes : " << arr.num_bytes() << std::endl;
        void *mapView = mmap(NULL, sb.st_size, PROT_READ, MAP_SHARED, fd, 0);
        ...
        arr.data_holder = std::make_shared<MemmapContainer>(fd, mapView, sb.st_size, arr.num_bytes());
        fclose(fp);
        return arr;
    }
}