...
namespace cnpyMmap
{
    struct MemmapContainer
    {
        ...
        MemmapContainer(int fd, void* start, size_t num_bytes, size_t arr_bytes): 
            _fd{fd}, _start{start}, _num_bytes{num_bytes}, _arr_bytes{arr_bytes} {
                ...
                _data = static_cast<char *>(_start) + _header_bytes;
            };
        ~MemmapContainer() {
            if (_start) {
                munmap(_start, _num_bytes);
            }
            if (_fd != -1) close(_fd);
        };
    };
    ...
    struct NpyArray
    {
        ...
        T *data()
        {
            return reinterpret_cast<T *>(data_holder->_data);
        }
    ...
    void npy_save(std::string fname, const T *data, const std::vector<size_t> shape, std::string mode = "w")
    {
        ...
        if (fp)
        {
            ...
            size_t word_size;
            bool fortran_order;
            read_npy_header(fp, word_size, true_data_shape, fortran_order);
